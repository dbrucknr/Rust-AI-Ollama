...
pub fn router() -> Router {
    Router::new()
        .route("/sse", post(stream_response))
...
async fn stream_response(
    ...
    let request = CompletionRequest {
        preamble: Some(String::from("You are a kind friend")),
        chat_history: OneOrMany::one(Message::user(body.content)),
        documents: Vec::new(),
        tools: Vec::new(),
        temperature: None,
    ...
    let sse_stream = result.map(|chunk_result| match chunk_result {
        Ok(chunk) => match chunk {
            StreamedAssistantContent::Text(text) => Ok(Event::default().data(text.text)), // Sends a string formatted as data: <content>
            StreamedAssistantContent::Final(_) => Ok(Event::default()),
